...
// Matrix format: CSR
__kernel void spmv(const int size, __global const cfloat *aValues,
                   __global const int *aPointers, __global const int *aCols,
                   __global const cfloat *x, __global cfloat *y,
                   __local cfloat *localSums, const int nRhs) {
  const int __idx = get_global_id(0);     // global thread index
  const int __localIdx = get_local_id(0); // local thread index
  const int threadLane =
      __localIdx & (WAVE_SIZE - 1);     // thread index within the wave
  ...
  for (int r = 0; r < nRhs; r++)
    sum[r] = (cfloat)(0.0f, 0.0f);
  for (int j = row_start + threadLane; j < row_end; j += WAVE_SIZE)
    for (int r = 0; r < nRhs; r++)
      sum[r] = cadd(sum[r], cmul(aValues[j], x[aCols[j] + r * size]));
